...
The next review pass also hardened protected file paths against case aliases on macOS, reserved the paper/review directory names from regular-file writes, and disabled curl’s default configuration before applying download policy. Regression coverage includes uppercase corpus/review paths and a curlrc that attempts to enable redirects; all 122 tests and the build still pass.

Literal IPv6 corpus URLs now omit curl’s DNS override because the address is already explicit. A loopback IPv6 regression verifies successful downloads and continued private-address enforcement; all 123 tests and the production build pass.

A further review pass added an aggregate cache budget and free-space floor, rejected backslash write aliases, made prepared-file copies atomic, and replaced PATH-dependent memory polling with platform-specific process measurements. All 126 tests and the build pass, including interrupted-copy recovery, cumulative storage accounting, reserved extraction space, backslash review-path rejection, and memory measurement with an empty PATH. The memory measurement was exercised on macOS locally; Linux CI will exercise `/proc`, while Windows PowerShell remains unverified on a Windows host.

Shared writes now enforce a configurable 1 GiB content budget, a 10,000-entry cap including directories, and a 2 GiB free-space floor with allocation headroom. The separately bounded paper cache is excluded. Structured review saves use the same storage check, and numbered review paths reject descendants before creating directories. All 128 tests and the production build pass, including budget rejection before mutation, replacement accounting, directory-count limits, and a peer attempting to block a pending review with a nested path.

Bulk preparation now publishes the required hosting manifest and registers the validated source experiment for local reuse after all 2,000 PDFs are present. It also saves the fixed corpus list for subsequent presets, excludes unrelated files from completion counts, and protects the generated manifest from agent edits. All 129 tests and the build pass. The publication regression covers incomplete downloads, CSV quoting/newlines, the full hosted schema, and source-identity validation.

Windows trailing-dot/space aliases are rejected before shared-write authorization, with regression coverage for review files, imported lists, and directory components. All 129 tests and the build pass. The security suggestion to add a non-disableable execution ceiling was assessed and intentionally declined because the user explicitly requested unlimited runs; optional positive limits and manual pause remain supported.

Agent paper listings now return review file paths instead of complete review bodies, keeping a ten-paper result small even when every review approaches 200 KB. Full reviews remain in exports and paged shared-file reads. arXiv listing fetches now disable curl’s default configuration, matching the PDF downloader. All 130 tests and the build pass, including ten large reviews retrieved through the real scheduler tool path.
...
Paper-read activity now keeps bounded metadata instead of source pages, and shared-write activity omits durable file contents. Runtime history and persisted activities use the same compaction; legacy read rows are projected compactly in SQLite before snapshot materialization. Session JSON exports stream one record at a time rather than loading all review bodies at once. All 135 tests and the build pass, including full text delivery to the agent, compact runtime/persisted/legacy activity, and large-review export shape/chunk checks. The browser download still uses a blob, and a paused experiment provides a stable export.

Hosted downloads now pin all validated DNS addresses, allowing fallback when the first address is unreachable. A curl 8.4+ preflight ensures unknown-length bodies obey the byte cap, with a final file-size check. All 136 tests and the build pass, including localhost with an IPv4-only listener and an oversized chunked response. The suggestion to remove generic web-fetch access was assessed as a permission tradeoff and declined to preserve the user-authorized research capability; this does not claim to eliminate prompt-injection-driven public egress.

Queued agent paper downloads now check session/reviewer cancellation generations and current pause/shutdown state before entering the cache worker. Experiment pause, reviewer interruption, and shutdown invalidate waiting work; already-started work may finish, and the operator’s separate bulk downloader remains independent. All 137 tests and the build pass, including experiment and reviewer pause before queued jobs perform filesystem or network work.
