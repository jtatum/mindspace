...
Use Node.js 24 or newer; this implementation was exercised with Node.js 26.8.1. The backend uses the built-in `node:sqlite` module. Paper downloads require curl 8.4 or newer on PATH so unknown-length responses respect the byte cap; the downloader checks this prerequisite and verifies the final file size. Install the pinned Codex CLI version, **0.154.0**, and make it available as `codex` on `PATH`, or select its executable with `MINDSPACE_CODEX_BIN`.
...
7. Pause the entire experiment or an individual agent when needed. Pause requests interruption of active model turns, cancels upcoming rounds, and rejects new agent tool calls. Queued agent paper downloads are cancelled before they start, including after a later resume. A file write or download already in progress may finish; completed work is not rolled back. Resume continues with saved context and pending work, starting a new turn rather than continuing the interrupted generation. Messages sent while paused remain saved and wait for resume. Export the experiment from the toolbar to save its observation record as JSON.

Paper/source read results are delivered fully to agents but stored as compact activity metadata. Shared-write activity omits saved content, which remains in the files. Legacy read activities are compacted when snapshots are selected. Session JSON exports stream records from SQLite, including complete paper reviews; the browser still collects the downloaded JSON as a blob. Pause the experiment for a stable export while no new records are being written.
...
| --- | --- | --- |
| `MINDSPACE_SHARED_MAX_BYTES` | `1073741824` (1 GiB) | Aggregate shared-file content budget per experiment, excluding the separately budgeted paper cache. Shared writes cap files/directories at 10,000 entries and preserve 2 GiB free disk space. |
| `MINDSPACE_PAPER_CACHE_MAX_BYTES` | `21474836480` (20 GiB) | Aggregate paper-directory budget per experiment, including extraction output and interrupted files. Downloads also preserve 2 GiB of free disk space. |
| `MINDSPACE_PAPERS_ALLOW_PRIVATE` | Unset | Set to `true` only for an intended private corpus hostname. Literal IPs and localhost explicitly identify trusted endpoints. |
...
Every experiment now has a shared working directory at `.mindspace/experiments/<session-id>/shared/`. Agents retain independent Codex homes and contexts but use the same working directory. Scoped file tools allow listing, paged text reads, and writes with revision checks to prevent overwriting unseen peer edits. The UI’s Shared files button shows its absolute path and files. The imported `papers.csv` and `papers.jsonl` are read-only to agents. Protected names are checked independent of case; writes reject backslashes and components ending in dots/spaces, and reserve the `papers` and `reviews` directory names. Reviewers read the extracted paper text with `read_shared_file` and save Markdown reviews with `write_shared_file` at `reviews/NNNN.md`. The file is saved exactly as written and the same transaction updates the paper queue; only the assigned reviewer can change that numbered review. Numbered review paths cannot be used as directories. Shared-file and structured-review writes check aggregate workspace storage and available disk space before writing. Revision checks protect against stale edits. Other agents can read the reviews and write shared notes or synthesis files. `record_paper_review` remains available, including for unavailable sources. File listings accept a directory path and paging offset, so the full corpus and review collection are accessible.

The paper tools expose bounded pages and enforce review ownership. Agent `read_papers` listings omit review bodies and return `reviewPath` for paged reading with `read_shared_file`; human API responses and exports retain full reviews. `cache_paper` saves a PDF under `papers/0001.pdf`, extracts text to `papers/0001.txt`, and records extraction metadata beside it. Existing files are reused. Missing PDFs come from `MINDSPACE_PAPERS_BASE_URL/papers/NNNN.pdf` when configured, with the manifest checked against the saved paper before downloading. Original arXiv URLs remain citations. Downloads are serialized and paced, capped at 150 MB per PDF from the configured host (to accommodate the prepared corpus’s larger files) or 50 MB from arXiv when no host is configured. Downloads reject redirects and pin all checked DNS addresses so IPv4/IPv6 fallback remains available; private DNS destinations require the explicit configuration above. PDF parsing runs in a separate process with a 192 MiB V8 heap cap, a parent-polled 512 MiB resident-memory guard, and a two-minute timeout. Header validation reads only 1 KiB in the server process. Memory supervision uses `/proc` on Linux, the built-in `/bin/ps` on macOS, and Windows PowerShell’s process working-set measurement on Windows; it does not depend on a `ps` command in PATH. Downloads and prepared-file copies check the aggregate cache budget and free-space floor before writing, reserving space for extraction output. Temporary copies are renamed only on completion. Existing PDFs remain available when a storage limit or extraction failure stops new work. Cached PDFs and extracted text continue to be reused; the host only needs to supply the manifest and PDFs. Text extraction stops at 300 pages or two million characters and reports truncation. Agents must distinguish sections actually read, missing sources, and extraction limitations. A downloaded PDF can still be useful when text extraction fails.
...
The command saves `download-status.json`, retries transient failures with delays, and preserves completed PDFs. Rerunning resumes from existing files. Three consecutive download failures stop the batch; extraction-only failures are recorded separately and do not prevent the remaining PDFs from downloading. Prepared PDFs are reused only when the source experiment’s saved paper number and URL match. Avoid running multiple bulk workers for the same corpus, or bulk downloading while agents are fetching papers. Once all 2,000 PDFs are present and their headers pass validation, the downloader atomically writes the five-column `shared/manifest.csv`, saves the fixed list at `corpora/ai-2000.json`, and registers the source at `corpora/download-experiment.json` for verified local reuse. An incomplete download does not publish a new manifest or source marker. Text-extraction failures do not block hosting completed PDFs. Serve only the experiment’s `shared/` directory when hosting the files locally; it contains no Codex login material. `.mindspace/` remains ignored by Git. No local hosting server is started automatically.
