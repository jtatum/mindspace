...
The command saves `download-status.json`, retries transient failures with delays, and preserves completed PDFs. Rerunning resumes from existing files. Three consecutive download failures stop the batch; extraction-only failures are recorded separately and do not prevent the remaining PDFs from downloading. Prepared PDFs are reused only when the source experiment’s saved paper number and URL match. Avoid running multiple bulk workers for the same corpus, or bulk downloading while agents are fetching papers. Serve only the experiment’s `shared/` directory when hosting the files locally; it contains no Codex login material. `.mindspace/` remains ignored by Git. No local hosting server is started automatically.
