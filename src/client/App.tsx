  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
      ...
      <nav className="session-list" aria-label="Experiments">
        {sessions.map(s => <button key={s.id} className={`session-link ${snapshot?.session.id === s.id ? 'selected' : ''}`} onClick={() => void openSession(s.id)}><span className={`session-dot ${s.status}`} /><span>{s.title}</span>{snapshot?.session.id === s.id && <ChevronDown size={13} />}</button>)}
    ...
    <div className="main-shell">
      ...
      {loading ? <div className="loading-state"><LoaderCircle className="spin" size={26} /><p>Opening your experiment…</p></div> : snapshot ? <>
        <section className="session-heading"><div><div className="eyebrow">COLLABORATIVE EXPERIMENT <span> / {snapshot.session.id.slice(0, 6).toUpperCase()}</span></div><h1>{snapshot.session.title}</h1><p>{agents.length} independent minds. One shared space.</p></div><div className="session-heading-right"><div className="avatar-stack">{agents.map(agent => <Avatar key={agent.id} participant={agent} />)}<span className="human-count"><Users size={13} />{humans.length}</span></div><span className="model-pill">Terra 5.6 <span>HIGH</span></span></div></section>
...
function CreateDialog({ onClose, onSubmit, mode }: { onClose: () => void; onSubmit: (input: CreateSessionInput) => Promise<void>; mode?: string }) {
