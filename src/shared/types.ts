...
export const DEFAULT_SETTINGS: Settings = { roundDelayMs: 10000, turnTimeoutMs: 180000, maxRounds: 20, maxTurns: 100, maxTokens: 200000, maxDurationMs: 3600000 };
...
export interface Participant { id: string; sessionId: string; kind: 'human' | 'agent'; name: string; color: string; instructions: string; model: string; effort: string; webFetch: boolean; status: AgentStatus; threadId: string | null; tokensUsed: number | null; groupCursor: number; pausedByHuman?: boolean }
...
export interface Snapshot { session: Session; participants: Participant[]; conversations: Conversation[]; messages: Message[]; activities: Activity[]; rounds: Round[]; deliveries: Delivery[]; eventSeq: number }
